class TypewriterWithTags {
	...
	static async createWithProcessing(element) {
		...
		const cleanInnerHTML = this.interpretJSLines(element.innerHTML);
		console.log(cleanInnerHTML);

		// Highlight text using Prism.js 
		const highlightedCleanInnerHTML = Prism.highlight(cleanInnerHTML, Prism.languages.javascript, 'javascript');
		element.innerHTML = highlightedCleanInnerHTML;

		// Output result using TypewriterWithTags
		new TypewriterWithTags(element);
	}

	static interpretJSLines(codeWithMarkers) {
		const originalHTML = codeWithMarkers;
		console.log(originalHTML);
		let replacedCode = '' 

		// Check if there is js code to interpret
		const needReplacement = codeWithMarkers.search(/\[\[[^\[\[]*\]\]/g);
		if (needReplacement) {
			replacedCode = codeWithMarkers.replaceAll(/\[\[[^\[\[]*\]\]/g, function (codeString) {
				codeString = codeString.slice(2, -2);
	...
	parseHTML() {
		...
		// console.log('Original HTML:', this.HTML);
	...
	traverseNodes(sourceNode, targetParent) {
		for (let child of sourceNode.childNodes) {
			if (child.nodeType === Node.TEXT_NODE) {
				...
				const txtCont = child.textContent;
				if (txtCont.length > 0) {
					const textElement = document.createTextNode('');
					targetParent.appendChild(textElement);

					// Check if there is js code to interpret
					const startInd = txtCont.search('\\[\\[');
					let replacedTxtCont = null;
					if (startInd !== -1) {
					...
					this.textNodes.push({
						element: textElement,
						originalText: replacedTxtCont ?? child.textContent,
						currentText: '',
...
document.addEventListener('DOMContentLoaded', function () {
	...
	Prism.hooks.add('after-highlight', function(env) {
		console.log('Prism: "Completed highlighting procedure"');
	...
	const codeBlocks = document.querySelectorAll('code.language-javascript, code.language-php, code[class*="language-"]');
	codeBlocks.forEach(block => {
		TypewriterWithTags.createWithProcessing(block);
	});
});

// Fallbacks method in case hooks don't work
/* window.addEventListener('load', function () {
	setTimeout(initTypewriterAfterPrism, 500);
}); */
